...
class ARCFile(object):
    ...
    def _write_header(self):
        ...
        if self.version == 1:
            payload = "1 0 %(org)s\nURL IP-address Archive-date Content-type Archive-length" % dict(
                org=self.file_headers['org'])
        elif self.version == 2:
    ...
    def _read_file_header(self):
        ...
        if self.version and int(self.version) != version:
            raise IOError(
                "Version mismatch. Requested version was '%s' but version in file was '%s'" % (self.version, version))
